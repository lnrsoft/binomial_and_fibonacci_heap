...
template <class T>
void heap<T>::cut(pForestNode node){
  if(node->father != NULL){
    pForestNode aux = node->father;
    if(aux->sons == node){
      //node es el hijo mayor
      if(node != node->broForward)
	aux->sons = node->broForward;
      else
	aux->sons = NULL;
    }
    unlink(node);
    node->marca = NO_MARCADO;
    link(elems, node);
    aux->range--;
    if(aux->marca)
      cut(aux);
...
bool heap<T>::check(const pForestNode & node, int range) const{
  ...
  pForestNode first = aux;
  for(int i=0; aux != first; i++, aux = aux->broForward){
    estado = estado && range > aux->range &&aux->broBackward->broForward == aux && aux->broForward->broBackward == aux && aux->father == node && check(aux, aux->range);
    if(!estado){
